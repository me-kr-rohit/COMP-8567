...
void processClientRequest(int sd)
{
    ...
    while (1)
    {
        ...
        rawCommand[n] = '\0';
        printf("COMMAND RECEIVED AS: %s,\nNOW PROCESSING THE REQUEST IN SERVER....\n", rawCommand);
        char roabResult[255];
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        fprintf(stderr, "INVALID SERVER START COMMAND...\nFORMAT-> %s <PORT>\n",argv[0]);
        exit(0);
    }
    ...
    while (1)
    {
        ...
        printf("<<-------------------------------------------------------------->>\n");
        // MIRROR LOGIC RUN IF CONDITION IS TRUE
        if ((roabNumOfConnections > 6 && roabNumOfConnections < 13) || (roabNumOfConnections > 12 && roabNumOfConnections % 2 == 0)) { 
            int clientDescriptor1 = accept(sd, (struct sockaddr *)NULL, NULL);
            printf("THIS IS CLIENT REQUEST NUMBER-> %d\n", roabNumOfConnections);
            printf("SERVER CAN'T PROCEED->REJECTING THIS CLIENT CONNECTON, TRY CONNECTING TO MIRROR SERVER IP and PORT!!!\n");
            char terminateMessage[] = "CONNECTION HAS BEEN TERMINATED/REJECTED BY THE MAIN SERVER KINDLY NOW TRY CONNECTING TO MIRROR IP & PORT.\n";
            write(clientDescriptor1, terminateMessage, strlen(terminateMessage) + 1);
            close(clientDescriptor1);
        }
        else // SERVER LOGIC
        {
            clientDescriptor = accept(sd, (struct sockaddr *)NULL, NULL);
            printf("CLIENT REQUEST NUMBER %d CONNECTION ESTABLISHED SUCCESSFULLY!!\n", roabNumOfConnections);
